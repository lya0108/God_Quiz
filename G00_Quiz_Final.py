...
class Play:
    def __init__(self, how_many, difficulty):
        ...
        self.stats_score = Label(self.score_frame, text="Score: 0%", bg="#ffe291")
        self.stats_score.grid(row=0, column=0, padx=37, pady=10)

        file = open("Highscore.txt", "r+")
        get_high_score = file.read()
        file.close()
        self.high_score = Label(self.score_frame, text=f"High Score: {get_high_score}", bg="#ffe291")
    ...
    def highscore(self, new_hisc):
        file = open("Highscore.txt", "r+")
        high_score = file.read()
        if high_score == "":
            high_score = "0"
        if new_hisc > int(high_score):
            file.seek(0)  # goes back to the start of the file
            file.write(str(new_hisc))  # writes new high score
...
class DisplayHelp:
    def __init__(self, partner):
        ...
        self.help_box.geometry("314x425")
